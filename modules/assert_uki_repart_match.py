...
def extract_uki_cmdline_params(ukify_json: dict) -> dict[str, str]:
    ...
    Exits early if "roothash" is not included.
    """
    ...
    if "roothash" not in params:
        print(
            f"UKI cmdline does not contain a roothash:\n{cmdline}"
        )
        exit(1)
...
def check_partitions(
    ...
    """
    Checks if the roothash from `uki_params` has a matching roothash
    for the corresponding partition in `partitions`.
    """
    for part in partitions:
        if part.get("type") == store_verity_type:
            expected = uki_params["roothash"]
