...
let
  ...
  partitionTypes = {
    ...
    root-verity =
      {
        "x86_64" = "root-x86-64-verity";
        "aarch64" = "root-arm64-verity";
      }
      .${pkgs.stdenv.hostPlatform.linuxArch};
  };
    ...
    pkgs.buildPackages.writers.writePython3Bin "assert_uki_repart_match.py"
      ...
      (
        builtins.replaceStrings
          [ "@NIX_STORE_VERITY@" ]
          [
            partitionTypes.root-verity
          ]
          (builtins.readFile ./assert_uki_repart_match.py)
      );
in
{
  options.image.repart.verityRoot = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    image.repart.partitions = {
      ...
      ${cfg.partitionIds.store-verity}.repartConfig = {
        Type = lib.mkDefault partitionTypes.root-verity;
        Verity = "hash";
      ...
      ${cfg.partitionIds.store} = {
        ...
        repartConfig = {
          Type = lib.mkDefault partitionTypes.root;
          Verity = "data";
    ...
    system.build = {
      ...
      # UKI with embedded roothash from intermediateImage
      uki =
        let
        ...
        lib.mkOverride 99 (
          pkgs.runCommand ukiFile
            ...
            ''
              ...
              # Extract the roothash from the output of the systemd-repart invocation for the intermediate image.
              roothash=$(jq -r \
                '.[] | select(.type=="${partitionTypes.root-verity}") | .roothash' \
                ${config.system.build.intermediateImage}/repart-output.json
              )

              # Build UKI with the embedded roothash.
              ukify build \
